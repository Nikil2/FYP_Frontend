...
export default function AdminUsersPage() {
  const [users, setUsers] = useState<AdminUserRow[]>(adminUsersSeed);
            ...
            <tbody>
              {filteredUsers.map((user) => (
                <tr key={user.id} className="border-b border-border/70 text-sm">
                  ...
                  <td className="px-4 py-3">
                    {user.isVerified ? (
                      ...
                      <span className="inline-flex items-center gap-1 text-amber-700">
                        <Filter className="h-4 w-4" /> Pending
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-paragraph">{user.createdAt}</td>
                  <td className="px-4 py-3 text-paragraph">{user.lastActive}</td>
                  <td className="px-4 py-3">
                    <Button
                      size="sm"
                      variant="outline"
